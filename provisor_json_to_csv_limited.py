# provisor_json_to_csv_limited.py
import csv
import os
import time
...
INPUT_JSON = r"exports/provisor_filial_1052_2026-05-14_04-21-14.json"  # замени на свой путь
OUT_DIR = "exports"

MAX_ROWS = 20000  # сколько строк обработать: 10000, 20000, 50000 и т.д.
...
def main():
    ...
    out_csv = os.path.join(OUT_DIR, f"provisor_emity_sample_{MAX_ROWS}_{now}.csv")
    ...
    print(f"[LIMIT] MAX_ROWS={MAX_ROWS}")
    ...
    with open(INPUT_JSON, "rb") as f_in, open(
        ...
        for item in ijson.items(f_in, "item"):
            ...
            if count >= MAX_ROWS:
                print(f"[STOP] reached MAX_ROWS={MAX_ROWS}")
                break
