...
def export_price_list_analytics(
    ...
    rows = [
        ...
        ["Snapshot", "yes" if payload.get("snapshot") else "legacy/no snapshot"],
        ["Snapshot schema", (payload.get("snapshot") or {}).get("schemaVersion", "")],
        ["Generated by", payload["priceList"].get("generatedBy", "")],
        ...
        ["Percentile usage", payload["summary"].get("percentileUsage", 0)],
        ["Substitute usage", payload["summary"].get("substituteUsage", 0)],
        ["Changed price count", payload["summary"].get("changedPriceCount", 0)],
        ["Price increased", payload.get("repricing", {}).get("increasedCount", 0)],
            ...
            if price_list is not None and result.get("skipped_unchanged"):
                try:
                    mark_unified_price_list_checked(
                        ...
                        message="Checked successfully; source price list is unchanged",
                    )
                except Exception:
                    logger.exception(
                ...
                try:
                    ...
                    if not items and int(result.get("localItemsCount") or 0) > 0:
                        mark_unified_price_list_checked(
                            ...
                            message="Checked successfully; zero-item response preserved existing rows",
                        )
                        refreshed.append(
                            {
                                "sourceType": account_source_type,
