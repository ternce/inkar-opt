...
export function CompetitorAssignmentTab({ formatCode, branch, priceFormats, onFormatChange, onNavigate }: Props) {
  ...
  const regionOptions = useMemo(
    () => Array.from(new Set(
      availableSources
        .map((row) => String(row.branchName || row.region || '').trim())
        .filter(Boolean)
  ...
  return (
    <div className="compact-table-wrap">
      <table className="compact-table">
        <thead>
          <tr>{columns.map((column, index) => <th key={column} className={stickyLastColumn && index === columns.length - 1 ? 'sticky-action-column' : undefined}>{column}</th>)}</tr>
        </thead>
        <tbody>
          {rows.length ? rows.map((row, idx) => (
            <tr key={idx}>
              {row.map((cell, cellIdx) => <td key={cellIdx} className={stickyLastColumn && cellIdx === columns.length - 1 ? 'sticky-action-column' : undefined}>{cell}</td>)}
            </tr>
          )) : (
            <tr>
